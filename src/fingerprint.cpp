...
std::string generate_hashes(std::vector<std::pair<int, int>> &v_in)
{
  //sorting
...
std::vector<std::pair<int, int>> get_2D_peaks(cv::Mat data)
{
...
std::string fp::fingerprint(float *data, size_t data_size,
                            float samplerate)
{
  FS = samplerate;
